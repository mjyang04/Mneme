...
@MainActor
final class ActivityFolderWatcher: ObservableObject {
    ...
    func start(workspaceURLs: [URL], onChange: @escaping () async -> Void) {
        stop()
        guard !workspaceURLs.isEmpty else { return }

        self.onChange = onChange
        let batcher = ActivityEventBatcher(workspaceRoots: workspaceURLs)
        ...
        let paths = workspaceURLs.map(\.path) as CFArray
        let queue = DispatchQueue(label: "mneme.activity.fsevents")
        guard let stream = FSEventStreamCreate(
            kCFAllocatorDefault,
            { _, contextInfo, eventCount, eventPaths, _, _ in
                guard let contextInfo else { return }
                let watcher = Unmanaged<ActivityFolderWatcher>
